...
const TARGET_LOCATION = "Dumfries, DG1 & DG2 (including areas: town centre, Maxwelltown, Locharbriggs, Georgetown, Heathhall, Troqueer, Lincluden, Summerhill, Brownhills, Noblehill)"
const LOCATION_ILIKE = "%Dumfries%"
const LOCATION_ILIKE_2 = "%DG1%"
const LOCATION_ILIKE_3 = "%DG2%"
...
const SECTOR_SOURCES: Record<string, string> = {
  ...
  "Accommodation": "VisitScotland, Airbnb, Booking.com, Cottages.com, Sykes Cottages",
  "Trades & Construction": "Checkatrade, Rated People, TrustATrader, Companies House",
  "Automotive": "mot-testers.co.uk, Companies House, Google Maps",
  "Professional Services": "Law Society of Scotland, ICAS, NHS Choices, Care Inspectorate, Companies House",
  "Healthcare": "NHS Choices, Care Inspectorate Scotland, GPhC register, NHS Dumfries & Galloway",
  ...
  "Fitness & Leisure": "Google Maps, Facebook, ClubFinder Scotland, Leisure & Culture Dumfries",
...
async function getExistingNames(sector: string): Promise<string[]> {
  const [r1, r2, r3] = await Promise.all([
    supabase.from("prospects").select("business_name").eq("sector", sector).ilike("location", LOCATION_ILIKE),
    supabase.from("prospects").select("business_name").eq("sector", sector).ilike("location", LOCATION_ILIKE_2),
    supabase.from("prospects").select("business_name").eq("sector", sector).ilike("location", LOCATION_ILIKE_3),
  ])
  const names = new Set<string>()
  for (const r of [...(r1.data ?? []), ...(r2.data ?? []), ...(r3.data ?? [])]) names.add(r.business_name)
...
Your task: Find real businesses in the "${sector}" sector located in Dumfries (DG1 or DG2 postcode) that you can genuinely verify exist — meaning you can point to a real source (Google Maps, Yell, Facebook, Companies House, TripAdvisor, or sector directory). Do not invent or pad results. If you have found everything real that exists, return [].

Dumfries is a town of ~35,000 people — the largest town in D&G. There are many more businesses here than in the smaller towns. Work through the town systematically by area: town centre (High Street, Shakespeare Street, English Street, Friars Vennel), Maxwelltown, Locharbriggs, Georgetown, Heathhall, Troqueer, Lincluden, Summerhill. Look for businesses that may not have appeared in earlier passes by exploring less prominent streets, business parks (Heathhall Industrial Estate, Brooms Road), and side streets.

Geographic rule: The business's primary trading address must be in Dumfries town (DG1 or DG2). Do NOT include businesses based in Thornhill, Lockerbie, or other D&G towns that merely serve Dumfries.
...
async function insertProspects(sector: string, prospects: Prospect[]): Promise<number> {
  ...
  if (toInsert.length === 0) {
    console.log(`[${sector}] All prospects already exist or lack contact data — nothing to insert.`)
    return 0
  }
...
async function main() {
  console.log(`\n=== Nith Digital Phase 1 — Dumfries/DG1-DG2 Market Research ===`)
