...
const TARGET_LOCATION = "Canonbie, DG14 (including nearby villages: Newcastleton, Penton, Rowanburn, Gilnockie, Byreburnfoot)"
const LOCATION_ILIKE = "%Canonbie%"
const LOCATION_ILIKE_2 = "%DG14%"
...
async function getExistingNames(sector: string): Promise<string[]> {
  ...
  for (const r of [...(r1.data ?? []), ...(r2.data ?? [])]) names.add(r.business_name)
...
Geographic rule: The business's primary trading address must be in Canonbie OR Newcastleton themselves, or in one of these named villages within the DG14 postcode area: Penton, Rowanburn, Gilnockie, Byreburnfoot. Note: Newcastleton uses TD9 postcode but is within the D&G council area and should be included. Do NOT include businesses based in Carlisle, Hawick, or other towns that merely serve the area.

This is a very rural, sparsely populated area. Expect fewer businesses than larger towns — if there are genuinely only a handful of verifiable businesses across all sectors, that is the correct answer. Do not pad with unverifiable records.

If the sector genuinely has fewer than 3 businesses not already in the list, document why in the notes field of each record (e.g. "Sector thin for DG14 — very rural border area, population under 2,000"). Still return what you find.
...
async function main() {
  console.log(`\n=== Nith Digital Phase 1 — Canonbie/DG14 Market Research ===`)
