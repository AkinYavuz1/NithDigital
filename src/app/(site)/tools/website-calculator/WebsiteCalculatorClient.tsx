  ...
  if (showResults && pkg && support) {
    ...
    return (
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '48px 24px', animation: 'slideUp 0.4s ease forwards' }}>
        ...
        {/* Package badge */}
        <div style={{ display: 'inline-block', padding: '6px 16px', background: '#1A1A1A', color: '#E85D3A', borderRadius: 100, fontSize: 12, fontWeight: 700, marginBottom: 16 }}>
          {pkg.name} Package
        </div>

        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 32, color: '#1A1A1A', marginBottom: 4, fontWeight: 400 }}>
          £{pkg.priceMin.toLocaleString()} – £{pkg.priceMax.toLocaleString()}
        </h2>
        <p style={{ fontSize: 14, color: '#7A7A7A', marginBottom: 8 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 28 }} className="calc-results-grid">
          ...
          {/* What's included */}
          <div style={{ background: '#FAF8F5', borderRadius: 12, padding: '24px 28px' }}>
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 16, color: '#1A1A1A', marginBottom: 14, fontWeight: 400 }}>
              What&apos;s included
            </h3>
          ...
          {/* Summary */}
          <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.1)', borderRadius: 12, padding: '24px 28px' }}>
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 16, color: '#1A1A1A', marginBottom: 14, fontWeight: 400 }}>
              Summary
            </h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                <span style={{ color: '#7A7A7A' }}>Pages</span>
                <span style={{ fontWeight: 600, color: '#1A1A1A' }}>{pages}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                <span style={{ color: '#7A7A7A' }}>Package</span>
                <span style={{ fontWeight: 600, color: '#1A1A1A' }}>{pkg.name}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                <span style={{ color: '#7A7A7A' }}>Build time</span>
                <span style={{ fontWeight: 600, color: '#1A1A1A' }}>{pkg.buildTime}</span>
              </div>
              {supportInfo.price > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, paddingTop: 10, borderTop: '1px solid rgba(0,0,0,0.08)' }}>
                  <span style={{ color: '#7A7A7A' }}>Monthly support</span>
                  <span style={{ fontWeight: 600, color: '#1A1A1A' }}>£{supportInfo.price}/mo</span>
                </div>
              )}
            </div>
            <p style={{ fontSize: 12, color: '#7A7A7A', marginTop: 16, lineHeight: 1.6 }}>{pkg.description}</p>
          </div>
        </div>
          ...
          <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <div
              style={{
                ...
                color: s === step ? '#1A1A1A' : s < step ? '#fff' : '#9AA8B8',
                display: 'flex',
                alignItems: 'center',
            ...
            <span style={{ fontSize: 11, color: s === step ? '#1A1A1A' : '#9AA8B8', fontWeight: s === step ? 600 : 400 }}>
              {stepLabels[s - 1]}
            </span>
            {s < 4 && <div style={{ width: 20, height: 2, background: 'rgba(0,0,0,0.1)', margin: '0 2px' }} />}
          </div>
        ))}
      </div>
      ...
      <div style={{ background: '#FAF8F5', borderRadius: 16, padding: '36px' }}>
        ...
        {step === 1 && (
          <div>
            ...
            <p style={{ fontSize: 14, color: '#7A7A7A', marginBottom: 24 }}>This helps us suggest the right package.</p>
            {([
        ...
        {step === 2 && (
          <div>
            ...
            <p style={{ fontSize: 14, color: '#7A7A7A', marginBottom: 24 }}>Select everything that applies. Greyed out items are included in all packages.</p>
                ...
                return (
                  <label
                    ...
                    style={{
                      ...
                      gap: 10,
                      padding: '12px 14px',
                      border: `2px solid ${checked ? '#E85D3A' : 'rgba(0,0,0,0.12)'}`,
                      borderRadius: 8,
                      cursor: f.locked ? 'default' : 'pointer',
                      background: f.locked ? 'rgba(0,0,0,0.04)' : checked ? 'rgba(232,93,58,0.08)' : '#fff',
                      transition: 'all 0.15s ease',
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: 16,
                        borderRadius: 4,
                        border: `2px solid ${checked ? '#E85D3A' : 'rgba(0,0,0,0.25)'}`,
                        background: checked ? '#E85D3A' : 'transparent',
                        flexShrink: 0,
                        marginTop: 1,
                        display: 'flex',
                        alignItems: 'center',
                    ...
                    >
                      {checked && <span style={{ color: '#1A1A1A', fontSize: 10, fontWeight: 800 }}>✓</span>}
                    </div>
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 600, color: '#1A1A1A' }}>{f.label}</div>
                      {f.locked && <div style={{ fontSize: 10, color: '#9AA8B8' }}>Included</div>}
                      {f.addonMin && <div style={{ fontSize: 10, color: '#E85D3A' }}>+£{f.addonMin}–£{f.addonMax}</div>}
                    </div>
                  </label>
                )
              })}
        ...
        {step === 3 && (
          <div>
            ...
            <p style={{ fontSize: 14, color: '#7A7A7A', marginBottom: 24 }}>Think: home, about, services, contact, and any specific pages for each service.</p>
            {([
        ...
        {step === 4 && (
          <div>
            ...
            <p style={{ fontSize: 14, color: '#7A7A7A', marginBottom: 24 }}>Keep your site updated, secure, and performing well.</p>
            {(Object.entries(supportOptions) as [SupportPlan, { label: string; price: number; desc: string }][]).map(([val, opt]) => (
              <label key={val} style={radioStyle(support === val)}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: '#7A7A7A' }}>{opt.desc}</div>
                </div>
                {opt.price > 0 && (
                  <div style={{ fontSize: 14, fontWeight: 700, color: '#E85D3A' }}>£{opt.price}/mo</div>
                )}
