...
export default function VATCheckerClient() {
  ...
  const verdictConfig = {
    ...
    approaching: { color: '#E85D3A', bg: 'rgba(232,93,58,0.1)', icon: '🟡', title: "You're approaching the VAT threshold", subtitle: `You're £${gap.toLocaleString()} below the £90,000 threshold.` },
        ...
        <div>
          <div style={{ background: '#FAF8F5', borderRadius: 12, padding: 32, marginBottom: 24 }}>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#1A1A1A', marginBottom: 8 }}>Annual taxable turnover (last 12 months)</label>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#1A1A1A', marginBottom: 8 }}>Expected turnover in the next 30 days alone</label>
              <div style={{ position: 'relative' }}>
                <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#7A7A7A', fontWeight: 600 }}>£</span>
                <input type="text" value={next30Input} onChange={e => handleNext30(e.target.value)}
                  style={{ width: '100%', padding: '12px 14px 12px 28px', border: '1px solid rgba(0,0,0,0.15)', borderRadius: 8, fontSize: 15, color: '#1A1A1A', fontFamily: 'inherit' }} />
              </div>
            </div>

            <div>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#1A1A1A', marginBottom: 8 }}>Business trend</label>
              <div style={{ display: 'flex', gap: 8 }}>
                {(['growing', 'stable', 'declining'] as const).map(t => (
                  <button key={t} onClick={() => setTrend(t)} style={{ flex: 1, padding: '8px 12px', borderRadius: 8, border: '2px solid', borderColor: trend === t ? '#1A1A1A' : 'rgba(0,0,0,0.15)', background: trend === t ? '#1A1A1A' : 'white', color: trend === t ? '#FAF8F5' : '#7A7A7A', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', textTransform: 'capitalize' }}>
                    {t}
                  </button>
                ))}
              </div>
          ...
          <div style={{ background: verdictConfig.bg, border: `2px solid ${verdictConfig.color}`, borderRadius: 12, padding: 28, marginBottom: 24 }}>
            ...
            <p style={{ fontSize: 15, color: '#333333', lineHeight: 1.7, marginBottom: 0 }}>{verdictConfig.subtitle}</p>
          ...
          <div style={{ background: '#FAF8F5', borderRadius: 12, padding: 24, marginBottom: 24 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#7A7A7A', marginBottom: 8 }}>
              <span>£0</span><span>VAT threshold: £90,000</span>
            </div>
            <div style={{ background: 'rgba(0,0,0,0.1)', borderRadius: 100, height: 12, overflow: 'hidden' }}>
              <div style={{ height: '100%', borderRadius: 100, width: `${Math.min(100, (turnover / VAT_THRESHOLD) * 100)}%`, background: mustRegister ? '#c0392b' : approaching ? '#E85D3A' : '#27ae60', transition: 'all 0.3s ease' }} />
            </div>
            <div style={{ marginTop: 8, fontSize: 13, fontWeight: 600, color: '#1A1A1A' }}>
          ...
          {turnover > 0 && (
            <div style={{ background: '#FAF8F5', borderRadius: 12, padding: 24 }}>
              ...
              <p style={{ fontSize: 14, lineHeight: 1.7, color: '#7A7A7A', marginBottom: 16 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div style={{ background: 'white', borderRadius: 8, padding: 16 }}>
                  <div style={{ fontSize: 11, color: '#7A7A7A', marginBottom: 4 }}>Standard VAT collected (20%)</div>
                  <div style={{ fontSize: 20, fontWeight: 700, color: '#1A1A1A' }}>£{Math.round(standardVAT).toLocaleString()}</div>
                </div>
                <div style={{ background: 'white', borderRadius: 8, padding: 16 }}>
                  <div style={{ fontSize: 11, color: '#7A7A7A', marginBottom: 4 }}>Flat Rate paid (est. 12%)</div>
              ...
              <p style={{ fontSize: 12, color: '#7A7A7A', marginTop: 12 }}>Note: Flat Rate Scheme percentages vary by industry. 12% is an estimate for business services.</p>
            </div>
          )}
        </div>
          ...
          <div style={{ background: '#FAF8F5', borderRadius: 12, padding: 24 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
                ...
                <div key={k} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, padding: '6px 0', borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
