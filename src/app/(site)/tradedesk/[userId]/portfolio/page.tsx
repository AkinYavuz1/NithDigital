    ...
    <div style={{ minHeight: '60vh', background: '#F5F0E6' }}>
        ...
        <div style={{ maxWidth: 1080, margin: '0 auto', padding: '0 24px' }}>
          ...
          {user.website_url && (
            <a
              ...
              style={{ fontSize: 13, color: 'rgba(245,240,230,0.55)', textDecoration: 'none' }}
            >
              {user.website_url.replace(/^https?:\/\//, '')}
            </a>
          )}
          <p style={{ fontSize: 13, color: 'rgba(245,240,230,0.45)', marginTop: 12 }}>
      ...
      <section style={{ maxWidth: 1080, margin: '0 auto', padding: '40px 24px 64px' }}>
        {!posts || posts.length === 0 ? (
          <p style={{ color: '#5A6A7A', fontSize: 14 }}>No portfolio posts yet.</p>
        ) : (
          <div style={{
            display: 'grid',
            ...
            {posts.map((post) => (
              ...
              >
                ...
                <div style={{ padding: '16px 18px' }}>
                  {post.ai_caption && (
                    <p style={{ fontSize: 13, color: '#1B2A4A', lineHeight: 1.6, margin: 0 }}>
                      {post.ai_caption}
                    </p>
                  )}
                  <p style={{ fontSize: 11, color: '#5A6A7A', marginTop: 8, margin: post.ai_caption ? '8px 0 0' : 0 }}>
      ...
      {/* Footer credit */}
      <div style={{ textAlign: 'center', padding: '0 24px 32px', fontSize: 12, color: '#5A6A7A' }}>
        Portfolio powered by{' '}
        <a href="https://nithdigital.uk/tradedesk" style={{ color: '#D4A84B', textDecoration: 'none' }}>
          TradeDesk
        </a>{' '}
        from Nith Digital
      </div>
