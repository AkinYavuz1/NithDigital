      ...
      <section style={{ maxWidth: 1080, margin: '0 auto', padding: '56px 24px 40px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20, marginBottom: 56 }} className="analyst-grid">
          {SKILLS.map((s) => (
            ...
            >
              <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 6, color: '#1A1A1A' }}>{s.skill}</h3>
              <p style={{ fontSize: 13, lineHeight: 1.6, color: '#7A7A7A', margin: 0 }}>{s.detail}</p>
            </div>
          ))}
        </div>
          ...
          {ENGAGEMENT_TYPES.map((e) => (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                <h3 style={{ fontSize: 14, fontWeight: 600, color: '#1A1A1A', margin: 0 }}>{e.type}</h3>
                <span style={{ fontSize: 12, fontWeight: 600, color: '#E85D3A', flexShrink: 0, marginLeft: 12 }}>{e.rate}</span>
              </div>
              <p style={{ fontSize: 13, lineHeight: 1.6, color: '#7A7A7A', margin: 0 }}>{e.desc}</p>
            </div>
          ))}
        </div>
