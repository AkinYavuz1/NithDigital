...
interface MileageLog { id: string; date: string; from_location: string; to_location: string; miles: number; purpose: string; rate_per_mile: number; total_claim: number }
...
export default function MileageClient() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      if (!data.user) return
      const { data: rows } = await supabase.from('mileage_logs').select('*').eq('user_id', data.user.id).order('date', { ascending: false })
      setLogs(rows || [])
      setLoading(false)
    })
  }, [])
  ...
  const totalClaim = logs.reduce((s, l) => s + Number(l.total_claim), 0)
  ...
  return (
    <div>
      <OSPageHeader title="Mileage" description="Track business journeys for tax relief"
        action={
          <Link href="/os/mileage/new" style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 18px', background: '#D4A84B', color: '#1B2A4A', borderRadius: 100, fontSize: 13, fontWeight: 600 }}>
      ...
      <div style={{ padding: 32 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 8, padding: '16px 20px', marginBottom: 24 }}>
          ...
          <div style={{ height: 8, background: 'rgba(27,42,74,0.1)', borderRadius: 100, overflow: 'hidden' }}>
            <div style={{ height: '100%', background: milesAt45p >= 10000 ? '#EF4444' : '#D4A84B', borderRadius: 100, width: `${Math.min(100, (milesAt45p / 10000) * 100)}%`, transition: 'width 0.4s ease' }} />
          </div>
          <p style={{ fontSize: 11, color: '#5A6A7A', marginTop: 8 }}>HMRC rate: 45p/mile for first 10,000 miles, 25p/mile thereafter</p>
        </div>

        {loading ? <div style={{ color: '#5A6A7A' }}>Loading...</div> : logs.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '64px 0', color: '#5A6A7A' }}>
            <div style={{ fontSize: 32, marginBottom: 12 }}>🚗</div>
            <p style={{ marginBottom: 16 }}>No journeys logged yet.</p>
            <Link href="/os/mileage/new" style={{ padding: '10px 20px', background: '#D4A84B', color: '#1B2A4A', borderRadius: 100, fontSize: 13, fontWeight: 600 }}>
