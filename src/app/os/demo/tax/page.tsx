...
export default function DemoTaxPage() {
  ...
  const grossIncome = data.invoices.filter(i => i.status === 'paid').reduce((s, i) => s + i.total, 0)
    + data.income.reduce((s, i) => s + i.amount, 0)
  const allowableExpenses = data.expenses.filter(e => e.is_allowable).reduce((s, e) => s + e.amount, 0)

  const [input, setInput] = useState<TaxInput>({ grossIncome, allowableExpenses, otherIncome: 0, studentLoanPlan: 'none' })
  const [result, setResult] = useState(() => calculateTax({ grossIncome, allowableExpenses, otherIncome: 0, studentLoanPlan: 'none' }))
  ...
  return (
    <div>
      <OSPageHeader title="Tax Estimator" description={`UK sole trader tax calculator — ${taxYear} (pre-filled from your demo data)`} />
      <div style={{ padding: 32, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 10, padding: 24 }}>
            ...
            {totalBands > 0 && (
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ height: 20, borderRadius: 4, overflow: 'hidden', display: 'flex' }}>
                  {bands.map(b => (
                    <div key={b.label} style={{ width: `${(b.amount / totalBands) * 100}%`, background: b.color }} title={`${b.label}: ${formatCurrency(b.amount)}`} />
                  ))}
                </div>
            ...
            <button onClick={saveEstimate} style={{ marginTop: 16, width: '100%', padding: '10px', background: saved ? '#F5F0E6' : '#1B2A4A', color: saved ? '#5A6A7A' : '#F5F0E6', borderRadius: 8, fontSize: 13, fontWeight: 600, border: 'none', cursor: 'pointer' }}>
              {saved ? '✓ Saved (demo only — sign up to save for real)' : 'Save this estimate'}
            </button>
          </div>
        </div>
      </div>
