...
export default function DemoBookingsPage() {
  ...
  const upcoming = data.bookings.filter(b => b.date >= new Date().toISOString().split('T')[0] && b.status === 'confirmed').length

  const updateStatus = (id: string, status: 'confirmed' | 'completed' | 'cancelled' | 'no_show') => {
    updateData(prev => ({
      ...prev,
    ...
    <div style={{ padding: '32px 40px', flex: 1 }} className="os-page-wrap">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 32 }} className="bookings-stats-grid">
        {[
          ...
          { label: 'Cancelled', value: data.bookings.filter(b => b.status === 'cancelled').length, color: '#5A6A7A' },
          { label: 'No-shows', value: data.bookings.filter(b => b.status === 'no_show').length, color: '#c0392b' },
        ].map(stat => (
              ...
              {filtered.map(b => (
                <tr key={b.id} style={{ borderBottom: '1px solid rgba(27,42,74,0.06)' }}>
                  <td className="td-primary" data-label="" style={{ padding: '12px 12px', fontWeight: 600 }}>
                    ...
                    <span style={{ marginLeft: 8 }}><span style={{ padding: '3px 10px', borderRadius: 100, fontSize: 11, fontWeight: 600, background: STATUS_COLORS[b.status]?.bg, color: STATUS_COLORS[b.status]?.color }}>{b.status.replace('_', '-')}</span></span>
                  </td>
