...
export default function AdminFinanceClient({
  ...
  // ── derived numbers ──────────────────────────────────────────────
  const starlingMonthlyEntries = starling ? Object.values(starling.monthly) : []
  const starlingTotals = starlingMonthlyEntries.length > 0
    ? starlingMonthlyEntries.reduce((acc, m) => ({ in: acc.in + m.in, out: acc.out + m.out }), { in: 0, out: 0 })
    : null
