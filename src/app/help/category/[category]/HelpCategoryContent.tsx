          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {articles.map(a => (
              <Link key={a.id} href={`/help/${a.slug}`} style={{ textDecoration: 'none' }}>
                <div className="help-cat-row" style={{ padding: '18px 20px', background: '#fff', borderRadius: 8, border: '1px solid rgba(27,42,74,0.08)', display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 15, fontWeight: 600, color: '#1B2A4A', marginBottom: 4 }}>{a.title}</div>
                    <div style={{ fontSize: 13, color: '#5A6A7A', lineHeight: 1.6 }}>{a.content.replace(/[#*`_]/g, '').trim().slice(0, 100)}...</div>
                  </div>
